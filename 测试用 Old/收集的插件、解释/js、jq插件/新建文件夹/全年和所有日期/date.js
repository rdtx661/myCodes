...
$(document).ready(function(){
        ...
        $('body *').click(function(event){
            ...
            function changeinputdata(dom){
                // $('.vch_02 .vch_02_show .datewarp .day_warp>div>div').click(function(){
                if(!$(dom).hasClass('bfm')&&!$(dom).hasClass('bfmn')){
                         var mydate=new Date();
...
function cgrl(dom,year,month){
        ...
        for(var i=1;i<=tempnum;i++){
                   ...
                   if((ttt+1)%7!=0){
                       ...
                       for(var c=(ttt+1)%7-1;c>=0;c--){
                            var domstr2=$(dom).parents('.vch_02').find('.day_warp').children('div:eq('+0+')').children('div:eq('+c+')');
                               ...
                               var mydate = new Date(); 
                               // 当前月及以后月天数划杠
                               if($(dom).parents('.fp_needdate').length>=1&&year2>=mydate.getFullYear()&&tempmonth>(mydate.getMonth()+1)){
                                        $(domstr2).addClass("bfm");
                               }else if($(dom).parents('.fp_needdate').length>=1&&year2<2012){
                                        $(domstr2).addClass("bfm");
                               }else{
                                    if(year2<mydate.getFullYear()&&$(dom).parents('.fp_needdate').length<1){
                                    //第一排非本年非本月内容
                                        $(domstr2).addClass("bfm");
                                    }else{    
                                    //第一排是本年非本月内容
            ...
            var domstr4=$(dom).parents('.vch_02').find('.day_warp').children('div:eq('+tempgo+')').children('div:eq('+tempgo2+')');
	      	$(domstr4).text(i);
            var mydate = new Date();   
            if($(dom).parents('.fp_needdate').length>=1&&year>=mydate.getFullYear()&&month>(mydate.getMonth()+1)){
                $(domstr4).addClass('bfmn');
            }else if($(dom).parents('.fp_needdate').length>=1&&year<2012){
                $(domstr4).addClass('bfmn');
            }else{
            	if(year<mydate.getFullYear()&&$(dom).parents('.fp_needdate').length<1){
                    $(domstr4).addClass('bfmn');
           		}
